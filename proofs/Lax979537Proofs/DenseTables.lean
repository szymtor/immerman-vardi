...
namespace Lax979537Proofs.DenseTables

open StackTuples

def dense {n k : Nat} (R : TableEvaluation.Table n k) : List Bool :=
  (Lax979537.StructureEncoding.tuples n k).map (fun v => decide (v ∈ R))
...
def predicate {n k : Nat} (p : (Fin k → Fin n) → Bool) (values : List Nat) : Bool :=
  (Lax979537.StructureEncoding.tuples n k).any (fun v => decide (tupleValues v = values) && p v)
...
def next {n k : Nat} (p : TableEvaluation.Table n k → (Fin k → Fin n) → Bool)
  ...
  (Lax979537.StructureEncoding.tuples n k).filter (p R)
...
end Lax979537Proofs.DenseTables
