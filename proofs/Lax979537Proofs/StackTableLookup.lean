...
namespace Lax979537Proofs.StackTableLookup
...
theorem lookup_returns (domain table index counter buffer tmp : K)
    ...
    (htable : s.stk table = (Lax979537.StructureEncoding.tuples n k).map p)
  ...
  have ht : t.stk table = (Lax979537.StructureEncoding.tuples n k).map p := by
  ...
  change Executes _ t (setStack (result
    (((Lax979537.StructureEncoding.tuples n k).map p)[rank n (tupleValues values)]?.getD false) t) index []) d at hq
...
end Lax979537Proofs.StackTableLookup
