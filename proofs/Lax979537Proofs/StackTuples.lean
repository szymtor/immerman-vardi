...
namespace Lax979537Proofs.StackTuples
...
theorem tuples_eq_canonical (n k : Nat) :
    tuples n k = (Lax979537.StructureEncoding.tuples n k).map tupleValues := by
  induction k with
  | zero => rfl
  | succ k ih =>
      rw [tuples, Lax979537.StructureEncoding.tuples, List.map_flatMap,
...
end Lax979537Proofs.StackTuples
